...
class RenewInsurancePolicyScreen extends StatefulWidget {
  ...
  @override
  State<RenewInsurancePolicyScreen> createState() => _RenewInsurancePolicyScreenState();
}

class _RenewInsurancePolicyScreenState extends State<RenewInsurancePolicyScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(widget.policy.insurancePackage?.name ?? 'N/A', style: _valueStyle),
            ),
            const Divider(height: 30, thickness: 1.2),
            const Text("Price:", style: _labelStyle),
            Padding(
              padding: const EdgeInsets.only(left: 24.0),
              child: Text("\$${widget.policy.insurancePackage?.price?.toStringAsFixed(2) ?? 'N/A'}", style: _valueStyle),
            ),
            const Divider(height: 30, thickness: 1.2),
            ...
            if (_selectedStartDate != null) ...[
              const Divider(height: 30, thickness: 1.2),
              Text("Start Date: ${formatDate(_selectedStartDate!)}", style: _infoStyle),
              Text("End Date: ${formatDate(_calculatedEndDate)}", style: _infoStyle),
            ],
            const Spacer(),
            Center(
            child: ElevatedButton.icon(
              ...
              label: _isRenewing
                  ? const Text("Renewing...", style: TextStyle(fontSize: 16))
                  : const Text("Renew Policy", style: TextStyle(fontSize: 16)),
              ...
              onPressed: _selectedStartDate == null || _isRenewing ? null : _renewPolicy,
            ),
          ),
  ...
  Future<void> _pickStartDate() async {
    ...
    if (pickedDate != null) {
      setState(() {
        ...
        _calculatedEndDate = pickedDate.add(Duration(days: widget.policy.insurancePackage!.durationDays!));
      });
    }
  }
  ...
  try {
    ...
    await insurancePolicyProvider.update(widget.policy.insurancePolicyId!, request);
